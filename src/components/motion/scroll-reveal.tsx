 ...
 * `blur`, not this.
 *
 * The scale and the translate are deliberately two separate tweens on two
 * separate elements: `data-reveal="pop"` (the whole tile) only ever
 * translates and fades, while a nested `[data-pop-scale]` (the cover alone)
 * is the only thing that grows. If the label scaled along with the icon, the
 * spring's overshoot would briefly blow the text past its own box and clip
 * it — this way the name is never touched by the spring at all.
 */
const START = 'top 85%'
const POP_EASE = 'back.out(1.08)'
    ...
    const start = async () => {
      ...
      if (popTargets.length > 0) {
        ...
        for (const [group, members] of groups) {
          ...
          const scaleEls = withOffsets.map(
            (o) => o.el.querySelector<HTMLElement>('[data-pop-scale]') ?? o.el,
          )

          gsap.set(els, { opacity: 0 })
          gsap.set(scaleEls, { scale: POP_SCALE_FROM })
          ...
          disposers.push(() => {
            ...
            gsap.set(scaleEls, { clearProps: 'transform' })
